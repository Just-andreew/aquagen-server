...
// 1. Authenticate Firebase from outside Google
const serviceAccount = {
    projectId: process.env.FIREBASE_PROJECT_ID,
    clientEmail: process.env.FIREBASE_CLIENT_EMAIL,
    // Replace string linebreaks with actual linebreaks for the RSA key
...
app.post('/telegramWebhook', async (req, res) => {
    try {
        ...
        if (!apiKey || !botToken) return res.status(200).send({ success: false });
        ...
        const geminiParts = [{ text: `You are an aquaculture AI triage engine for AquaGen... Analyze text/image. Return raw JSON. {"event_type": "...", "ponds": [], "metrics": {"feed_amount": null, "average_weight_g": null, "water_parameters": null}, "ai_visual_verification": "...", "confidence_score": 95} Message: "${combinedText}"` }];
        ...
        await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: chatId, text: `✓ <b>Log Saved</b>\n<b>Action:</b> ${aiData.event_type}\n<blockquote>"${combinedText}"</blockquote>`, parse_mode: "HTML" })
        });

        res.status(200).send({ success: true });
    } catch (error) { res.status(200).send({ success: false }); }
});

// ============================================================================
// MODULE 2: ESP32 POLL (MANUAL DROP)
// ============================================================================
app.get('/feederPing', async (req, res) => {
    try {
        const deviceId = req.query.device_id;
        if (!deviceId) return res.status(400).send({ error: 'Missing device' });
    ...
    } catch (e) { res.status(500).send({ drop: false }); }
});

// ============================================================================
// MODULE 3: ESP32 RECEIPT LOGGING
// ============================================================================
app.post('/iotLog', async (req, res) => {
    if (req.headers['x-iot-secret'] !== process.env.IOT_SECRET_KEY) return res.status(401).send({ error: 'Unauthorized' });
    ...
    try {
        ...
        const logEntry = {
            ...
            data: {
                ...
                metrics: { feed_amount: `${payload.grams_dispensed}g`, average_weight_g: null, water_parameters: null },
                ai_confidence: 100,
                notes: `Hardware success. Battery: ${payload.v_batt || 'N/A'}V`,
                original_text: `[IoT Receipt]: Dispensed=${payload.grams_dispensed}g`
            },
            source: "ESP32_IoT"
        };
        await db.collection('logs').add(logEntry);
        res.status(200).send({ success: true });
    } catch (error) { res.status(500).send({ error: 'DB Error' }); }
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`🚀 AquaGen Server running on port ${PORT}`));